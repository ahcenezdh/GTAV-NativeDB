...
export function convertTypeToLua(type: string, useNativeTypes: boolean, convertHashes = true) {
...
class LuaCodeGenerator extends CodeGeneratorBase<LuaCodeGeneratorSettings> {
  ...
  transformBaseType(type: string): string {
    return convertTypeToLua(type, this.settings.useNativeTypes, this.settings.convertHashes)
  }
